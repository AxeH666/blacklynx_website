...
- Text: `--foreground` `oklch(0.95 0 0)` (primary text/foreground) · `--text-secondary` `oklch(0.76 0 0)` (≥4.5:1 on all surfaces) · `--text-muted` `oklch(0.72 0 0)`
...
:root {
  ...
  --foreground:   oklch(0.95 0 0);   /* primary text/foreground — never pure white */
...
| Text primary | `--foreground` (Tailwind: `text-foreground`) |
...
| Flag | Item | Status |
|------|------|--------|
| #1 | Elevation token names (`surface`/`raised`/`overlay`/`foreground`) | **RECONCILED** |
